...
const CP = () => {
  ...
  const loadSolvedProblems = async () => {
    ...
    try {
      ...
      if (data.status === "OK") {
        ...
        setError("Could not find user. Please check the handle and try again.");
      }
    } catch (error) {
  ...
  return (
    <div className="min-w-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <Header title={"DSA Sheet"} subtitle={"Customized Learning"}/>
          ...
          <div className="max-w-lg mx-auto mb-8">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-300 rounded-lg
                  focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                  text-slate-900 placeholder-slate-400"
              />
            </div>
            <button
              onClick={loadSolvedProblems}
              disabled={loading}
              className="mt-3 w-full bg-blue-600 text-white py-2.5 px-4 rounded-lg
                hover:bg-blue-700 transition duration-200 flex items-center justify-center
                gap-2 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : null}
              {loading ? "Loading..." : "Load Solved Problems"}
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-8 justify-center">
            {ratings.map((rating) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all bg-blue-500 bg-cover
                  `}
              >
                {rating}
              </button>
